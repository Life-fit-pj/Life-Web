...
| **월별 가입 추이** | 103명 자료로는 월 3~9명 진동만 보입니다. 추세가 아니라 잡음인데 화면 폭 전체를 먹습니다 |
...
> 곧 제목 줄에 `회원 <b>103</b>명`을 넣을 건데, 그 `103`만 진하게 만드는 규칙입니다.
>
...
async function loadDashboard() {
  ...
  const genders = ch.genders.map(g => ({ ...g, label: g.label || '미입력' }));
...
원본은 시각을 **바로 화면에 찍었습니다.** 그런데 이제 `회원 103명 · (시각)`처럼
...
로그인 기능이 만든 계정 3개는 성별이 비어 있습니다(`null`). 서버는 `M→남성`, `F→여성`만
바꾸고 `null`은 그대로 보내서, 범례에 **이름 없는 항목**이 생깁니다.
...
- `g.label || '미입력'`은 "라벨이 비었으면 `'미입력'`을 쓴다"는 뜻입니다.
...
```
한눈에 보기  회원 103명 · 2026. 9. 2. 오후 11:40 기준          [새로고침]
...
```
회원 103명 · member_chunk 900개 (회원당 8.7칸, 정상은 9칸)
user_preferences 100행 · 나이 있는 회원 100명 · 성별 [없음×3, 여×50, 남×50]
```

`C101`·`C102`·`C103` — **로그인 기능이 발급한 계정 3개**가
...
셋이 전부 3을 가리킵니다 → **같은 계정 3개** 이야기입니다. 그래서 한 줄로 묶습니다.

만약 `noProfile`은 3인데 `asMembers`가 7.5처럼 **어긋나면**, 그건
...
> 그래서 `genders`의 합은 값이 비어도 103이 나와 버려 셀 수가 없습니다.
...
| 볼 것 | 기대 |
|---|---|
| 대시보드 | 카드 5장, 제목 줄에 `회원 103명`, 성별에 `미입력 3` |
| 시스템 | 이슈 1건 + 청킹/가중치 카드 |
