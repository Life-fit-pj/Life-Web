...
```
Life-Web/
├── main.py            FastAPI 서버 (/api/predict, /api/region,
│                       /api/region/explain, /api/chat)
├── services/          기능별 분리
│   ├── coords.py        행정동 좌표 조회
│   ├── engine.py        엔진 호출 + 한↔영 키 변환 + 시설 정보 조회
│   ├── floorplan.py     LH 평면도 선택
│   └── price.py         시세 조회 + 예산 초과분 감점
...
│   │   ├── state.js        화면 간 공유 상태(마지막 결과·검색어 등)
│   │   └── format.js       문자열 다듬기 순수 함수
│   └── ui/              화면 단위 JS + 그중 큰 화면은 짝이 되는 CSS
│       ├── search.js         첫 검색 화면 로직 + 상단 검색바
...
│       └── menu.js/.css      우측 상단 메뉴 패널
└── data/
    ├── 동_좌표.csv                    427개 행정동 위경도
    ├── 시세_지역별.csv                 동네별 시세(예산 감점용)
    ├── 시세_면적구간별.csv             면적 구간별 시세
...
- 실제 로그인/회원가입 API (메뉴의 로그인은 `localStorage` 토큰 유무만 보는
  개발용 임시 구현이며, 마이페이지·검색 기록 저장·좋아요 등은 전부 "준비 중"
  안내만 뜸)