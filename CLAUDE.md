...
추천해주는 Flask 서버 + 정적 프론트엔드입니다. 이 저장소는 UI와 HTTP 레이어**만** 담당합니다.
모든 추천 로직과 LLM 호출은 형제 저장소인 `life-fit-embed`에 있으며, 이 저장소는 `sys.path`를
통해 이를 직접 임포트합니다(pip 설치 방식이 아님).

## 필수 폴더 구조

이 저장소는 반드시 `life-fit-embed`와 같은 레벨에 위치해야 합니다:

```
Life_Fit/
├── life-fit-embed/   # 추천 엔진 (DB, LLM, 파이프라인) — 별도 저장소
└── life-fit-web/     # 이 저장소
```

`services/engine.py`는 `../life-fit-embed`를 `sys.path`에 추가하고 여기서
`app.features.pipeline_api.{search, recommend_by_weights}`와
`app.core.db.{facilities, facility_counts}`를 임포트합니다. `life-fit-embed`가 없거나 이름이
잘못되면 서버는 임포트 시점에 `ModuleNotFoundError: No module named 'app'` 오류로 실패합니다.
`life-fit-embed`는 자체 `.env`에 `ANTHROPIC_API_KEY`와 `data/life.db`(약 216MB, git에 포함되지
...
**요청 흐름:** `frontend/*.js` → Flask 앱(`main.py`)의 `POST /api/predict` 또는 `/api/region`
→ `services/engine.py`가 형제 패키지인 `life-fit-embed`를 호출 → 응답을 재구성해 JSON으로 반환
→ 프론트엔드가 지도 핀/카드를 렌더링.

- `main.py` — Flask 앱 설정, 정적 파일 서빙(프론트엔드 + `data/LH평면도` 이미지), 그리고 두 개의
  API 라우트를 담당합니다. 라우트 핸들러는 입출력 형태만 다듬고, 실제 작업은 모두
  `services/`에 위임합니다.
- `services/engine.py` — **`life-fit-embed`를 알고 있는 유일한 파일**입니다. 다른 추천 엔진으로
  교체하려면 여기 있는 두 개의 import 줄만 바꾸면 됩니다(아래 "교체 가능한 엔진 계약" 참고).
  또한 7개 라이프스타일 지표에 대한 한국어⇄영어 키 매핑도 이 파일이 담당합니다.
...
- `frontend/` — 빌드 단계도, 프레임워크도 없습니다. `index.html`/`search.css`/`search.js`는
  ...
  결과 화면(Kakao 지도, 순위 목록, 핀별 추천 사유 카드)입니다.
...
- `POST /api/predict` — 요청 본문은 `{ query: "..." }`(LLM이 텍스트를 7개 지표 가중치로 변환)
  ...
  슬라이더 값(가중치로 그대로 사용됨) 중 하나입니다. 두 경로 모두
  `services.engine.get_regions`로 수렴하며, 동일한 형태를 반환합니다: score, `topRegions`(좌표
  포함), `floorplanPath`, `explanation`, `weights`(프론트엔드가 슬라이더를 다시 동기화할 수
  있도록).
- `POST /api/region` — 요청 본문 `{ gu, dong }`, 지도 핀 클릭 시 표시되는 모달용으로 인근 시설
  개수/항목을 반환합니다. 핀 클릭 시 427개 동 전체 점수를 다시 계산하지 않도록 `/api/predict`와
  의도적으로 분리되어 있습니다.

### 교체 가능한 엔진 계약

어떤 엔진이든 아래 두 함수를 정확히 이 반환 형태로 노출하기만 하면 `life-fit-embed`를 대체할
...
추천 엔진이 사용하는 Anthropic 키는 이 저장소의 `.env`가 아닌 `life-fit-embed/.env`에
있습니다.
