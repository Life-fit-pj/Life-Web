...
`http://127.0.0.1:5000`에서 서비스된다. 사전 준비물: `Life-Embed-jh/.env`(`ANTHROPIC_API_KEY`)와
...
`services/engine.py`가 형제 패키지 `Life-Embed-jh`를 호출 → 응답을 JSON으로 재구성 →
프론트엔드가 지도 핀/카드를 렌더링.
...
- `services/engine.py` — **`Life-Embed-jh`를 아는 유일한 파일.** 다른 추천 엔진으로 교체하려면
  여기 import 줄만 바꾸면 된다(계약: `search(query, top_k)` / `recommend_by_weights(weights,
  top_k)`, 자세한 내용은 README 참고). 7개 지표(녹지·안전·교통·상권·의료·교육·문화)의
  한국어⇄영어 키 매핑도 여기 하나만 있다.
